...
public class DoctorSlot
{
    ...
    [MaxLength(100)] public string DoctorName { get; private set; } = string.Empty;
    ...
    public static DoctorSlot Create(DateTime date, Guid doctorId, string doctorName, decimal cost)
    {
        ...
        if (doctorId == default) throw new ArgumentException("DoctorId is required", nameof(doctorId));
        if (doctorName == string.Empty || doctorName == null)
            throw new ArgumentException("DoctorName is required", nameof(doctorName));
        if (cost == default) throw new ArgumentException("Cost is required", nameof(cost));
    ...
    public static DoctorSlot CreateReserved(DateTime date, Guid doctorId, string doctorName, decimal cost)
    {
        if (date == default) throw new ArgumentException("Date is required", nameof(date));
        if (doctorId == default) throw new ArgumentException("DoctorId is required", nameof(doctorId));
        if (doctorName == string.Empty) throw new ArgumentException("DoctorName is required", nameof(doctorName));
        if (cost == default) throw new ArgumentException("Cost is required", nameof(cost));
